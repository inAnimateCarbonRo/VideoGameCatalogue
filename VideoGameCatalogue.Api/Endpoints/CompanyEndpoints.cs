...
namespace VideoGameCatalogue.Api.Endpoints
{
    public static class CompanyEndpoints
    {
        ...
        {
            var group = app.MapGroup(ApiEndpoints.CompanyEndpoints.GetAll.Split('/')[..2].Aggregate((a, b) => $"{a}/{b}"))
                .WithTags("Companies");

            group.MapGet("/", GetAll)
                .WithName("GetAllCompanies")
                
                .Produces<CompanysResponse>(StatusCodes.Status200OK);
            ...
            group.MapGet("/all-including-deleted", GetAllIncludingDeleted)
                .WithName("GetAllCompaniesIncludingDeleted")
            ...
            group.MapDelete("/fulldelete/{id}", FullDelete)
                .WithName("FullDeleteCompany")
                
                .Produces(StatusCodes.Status204NoContent)
                .Produces(StatusCodes.Status404NotFound);
        }

        private static async Task<IResult> GetAll(ICompanyService service, CancellationToken token)
        {
        ...
        private static async Task<IResult> GetById(int id, ICompanyService service, CancellationToken token)
        {
        ...
        private static async Task<IResult> GetAllIncludingDeleted(ICompanyService service, CancellationToken token)
        {
        ...
        private static async Task<IResult> Create([FromBody] CreateCompanyRequest item, ICompanyService service, CancellationToken token)
        {
            ...
            try
            {
                ...
                return Results.CreatedAtRoute("GetCompanyById", new { id = entity.Id }, entity.MapToResponse());
            }
            catch (DbUpdateException ex) when (ex.InnerException is Microsoft.Data.SqlClient.SqlException sqlEx
                                              && (sqlEx.Number == 2601 || sqlEx.Number == 2627))
            {
                return Results.Conflict($"Company '{item.Name}' already exists.");
            }
        }

        private static async Task<IResult> Update(int id, [FromBody] UpdateCompanyRequest item, ICompanyService service, CancellationToken token)
        {
            if (item == null) return Results.BadRequest("Invalid data.");
        ...
        private static async Task<IResult> Delete(int id, ICompanyService service, CancellationToken token)
        {
        ...
        private static async Task<IResult> Restore(int id, ICompanyService service, CancellationToken token)
        {
        ...
        private static async Task<IResult> FullDelete(int id, ICompanyService service, CancellationToken token)
        {
